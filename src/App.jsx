                              ...
                              {group.gems.map((gem, si) => (
                                <div key={si} className="flex items-center justify-between">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <button onClick={() => window.open(searchGemTrade(selectedLeague, gem.name, gem.level, gem.quality || null), '_blank')} className="text-[8px] text-purple-400 hover:text-purple-300 font-bold px-1 rounded bg-purple-400/10 hover:bg-purple-400/20" title={`Buy ${gem.name} L${gem.level}${gem.quality ? '/Q' + gem.quality : ''} (as in guide)`}>{gem.level}/{gem.quality || 0}</button>
                                    <button onClick={() => window.open(searchGemTrade(selectedLeague, gem.name, 20), '_blank')} className="text-[8px] text-emerald-400 hover:text-emerald-300 font-bold px-1 rounded bg-emerald-400/10 hover:bg-emerald-400/20" title={`Buy ${gem.name} level 20`}>L20</button>
                                    <button onClick={() => window.open(searchGemTrade(selectedLeague, gem.name, 20, 20, { corrupted: 'any' }), '_blank')} className="text-[8px] text-blue-400 hover:text-blue-300 font-bold px-1 rounded bg-blue-400/10 hover:bg-blue-400/20 border border-red-500/50" title={`Buy ${gem.name} level 20 quality 20 (any corrupt)`}>20/20</button>
                                  </div>
                          ...
                          actives.map((gem, si) => (
                            <div key={si} className="flex items-center justify-between">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-[7px] text-slate-500 ml-1">⚑</span>
                                <button onClick={() => window.open(searchGemTrade(selectedLeague, gem.name, gem.level, gem.quality || null), '_blank')} className="text-[8px] text-purple-400 hover:text-purple-300 font-bold px-1 rounded bg-purple-400/10 hover:bg-purple-400/20" title={`Buy ${gem.name} L${gem.level}${gem.quality ? '/Q' + gem.quality : ''} (as in guide)`}>{gem.level}/{gem.quality || 0}</button>
                                <button onClick={() => window.open(searchGemTrade(selectedLeague, gem.name, 20), '_blank')} className="text-[8px] text-emerald-400 hover:text-emerald-300 font-bold ml-1 px-1 rounded bg-emerald-400/10 hover:bg-emerald-400/20" title={`Buy ${gem.name} level 20`}>L20</button>
                                <button onClick={() => window.open(searchGemTrade(selectedLeague, gem.name, 20, 20, { corrupted: 'any' }), '_blank')} className="text-[8px] text-blue-400 hover:text-blue-300 font-bold px-1 rounded bg-blue-400/10 hover:bg-blue-400/20 border border-red-500/50" title={`Buy ${gem.name} level 20 quality 20 (any corrupt)`}>20/20</button>
                              </div>
